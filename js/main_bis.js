      ...
      function updateCSSVariable(varName, value) {
        // agrega px si corresponde
        //  if (/^--.*(width|radius|size)$/i.test(varName) && /^[0-9.]+$/.test(value)) {
        //  value = value + "px";
        //  }
      ...
      document.addEventListener("DOMContentLoaded", async () => {
        try {
          await document.fonts.load('400 1rem "Edu NSW ACT Cursive"');
        } catch {}
        ...
        // asegurá que la familia (peso 400) esté lista; si falla, seguimos igual
        //  try { await document.fonts.load('400 1rem "Truculenta"'); } catch {}
        // no bloquea: browsers sin Font Loading API igual siguen
        //   initVarInputs();

        // ——— export ———
        document.getElementById("tailleauto").addEventListener("click", () => {
        ...
        document
          ...
          .addEventListener("click", async () => {
            ...
            // por si el navegador demora en activar la fuente
            //    try { await document.fonts.load('400 1rem "Truculenta"'); } catch {}

            const result = await snapdom.toImg(element, {
              embedFonts: true,
              scale: 1.1,
            });

            document.querySelector(".editor").appendChild(result);
          });
      });
    </script>
    <script type="module">
      ...
      function applyProxy(url, useProxy) {
        ...
        if (useProxy.includes("{url}"))
          return useProxy.replace("{url}", encodeURIComponent(url));
        const sep = useProxy.endsWith("?")
          ? ""
          : useProxy.includes("?")
            ? "&"
            : "?";
      ...
      async function inlineSingleBackgroundEntry(entry, options = {}) {
        ...
        try {
          const dataUrl = await snapFetch(encodedUrl, {
            as: "dataURL",
...
`;
      ...
      function _resolveCtx(sessionOrCtx, opts) {
        ...
        if (
          sessionOrCtx &&
          (sessionOrCtx.styleMap ||
            sessionOrCtx.styleCache ||
            sessionOrCtx.nodeMap)
        ) {
          return {
...
`;
      ...
      async function deepClone(node, sessionCache, options) {
        ...
        if (node.tagName === "IFRAME") {
          ...
          if (sameOrigin) {
            try {
              const wrapper = await rasterizeIframe(
                node,
                sessionCache,
                options,
              );
        ...
        if (node.shadowRoot) {
          ...
          const seed = buildSeedCustomPropsRule(
            node,
            neededVars,
            scopeSelector,
          );
          ...
          const cloneList2 = await idleCallback(
            Array.from(node.shadowRoot.childNodes),
            callback2,
            options.fast,
          );
          shadowFrag.append(
            ...cloneList2.filter((clonedChild) => !!clonedChild),
          );
      ...
      function isLikelyFontStylesheet(href, requiredFamilies) {
        ...
        try {
          ...
          if (/\bfont(s)?\b/.test(path) || /\.woff2?(\b|$)/.test(path))
            return true;
      ...
      async function embedCustomFonts({
        ...
        function dedupeFontFaces(cssText) {
          ...
          for (const block of cssText.match(FACE_RE2) || []) {
            const familyRaw =
              block.match(/font-family:\s*([^;]+);/i)?.[1] || "";
            const family = pickPrimaryFamily(familyRaw);
            const weightSpec = (
              block.match(/font-weight:\s*([^;]+);/i)?.[1] || "400"
            ).trim();
            const styleSpec = (
              block.match(/font-style:\s*([^;]+);/i)?.[1] || "normal"
            ).trim();
            const stretchSpec = (
              block.match(/font-stretch:\s*([^;]+);/i)?.[1] || "100%"
            ).trim();
            const urange = (
              block.match(/unicode-range:\s*([^;]+);/i)?.[1] || ""
            ).trim();
            const srcRaw = (
              block.match(/src\s*:\s*([^;]+);/i)?.[1] || ""
            ).trim();
        ...
        for (const link of linkNodes) {
          try {
            ...
            for (const face of cssText.match(FACE_RE) || []) {
              ...
              const weightSpec = (
                face.match(/font-weight:\s*([^;]+);/i)?.[1] || "400"
              ).trim();
              const styleSpec = (
                face.match(/font-style:\s*([^;]+);/i)?.[1] || "normal"
              ).trim();
              const stretchSpec = (
                face.match(/font-stretch:\s*([^;]+);/i)?.[1] || "100%"
              ).trim();
              const urange = (
                face.match(/unicode-range:\s*([^;]+);/i)?.[1] || ""
              ).trim();
              const srcRaw = (
                face.match(/src\s*:\s*([^;]+);/i)?.[1] || ""
              ).trim();
          ...
          for (const f of document.fonts || []) {
            ...
            if (!String(b64).startsWith("data:")) {
              if (cache.resource?.has(f._snapdomSrc)) {
                ...
                try {
                  const r = await snapFetch(f._snapdomSrc, {
                    as: "dataURL",
                    useProxy,
                    silent: true,
                  });
      ...
      function collectUsedFontVariants(root) {
        ...
        if (
          csBeforeRoot &&
          csBeforeRoot.content &&
          csBeforeRoot.content !== "none"
        )
      ...
      async function inlinePseudoElements(
        source,
        clone,
        sessionCache,
        options,
        ...
        for (const pseudo of ["::before", "::after", "::first-letter"]) {
          try {
            ...
            if (!style || typeof style[Symbol.iterator] !== "function")
              continue;
            const isEmptyPseudo =
              style.content === "none" &&
            ...
            console.warn(
              `[snapdom] Failed to capture ${pseudo} for`,
              source,
              e,
            );
          }
      ...
      function inlineExternalDefsAndSymbols(rootElement) {
        ...
        rootElement
          .querySelectorAll("symbol[id], defs > *[id]")
          .forEach((el) => {
      ...
      async function prepareClone(element, options = {}) {
        ...
        for (const [
          cloneNode,
          originalNode,
        ] of sessionCache.nodeMap.entries()) {
        ...
        const processImg = async (img) => {
          ...
          if (defaultImageUrl) {
            try {
              ...
              const fallbackUrl =
                typeof defaultImageUrl === "function"
      ...
      async function captureDOM(element, options) {
        ...
        let dataURL;
        let svgString;
        ({ clone, classCSS, styleCache } = await prepareClone(
          element,
          options,
        ));
        ...
        if (options.embedFonts) {
          console.log(options.embedFonts);
          await new Promise((resolve) => {
            idle(
              async () => {
          ...
          idle(
            () => {
              ...
              if (!hasBBoxAffectingTransform(element)) {
                ...
                const e = TOTAL.e ?? TOTAL.m41 ?? 0;
                const f = TOTAL.f ?? TOTAL.m42 ?? 0;
                cancelTranslateX = -e;
                cancelTranslateY = -f;
                const M2D = TOTAL.is2D
                  ? matrix2DNoTranslate(TOTAL)
                  : matrix2DNoTranslate(new DOMMatrix(TOTAL.toString()));
              ...
              if (cancelTranslateX || cancelTranslateY) {
                cancels.push(
                  `translate(${cancelTranslateX}px, ${cancelTranslateY}px)`,
                );
              }
        ...
        return {
          ...
          iconFonts: Array.isArray(options.iconFonts)
            ...
            : options.iconFonts
              ? [options.iconFonts]
              : [],
          localFonts: Array.isArray(options.localFonts)
            ? options.localFonts
            : [],
          excludeFonts: options.excludeFonts ?? void 0,
          defaultImageUrl: options.defaultImageUrl ?? void 0,
          /** Cache policy (disabled|full|auto|soft). Default: "soft" */
          /** @type {CachePolicy} */
          cache: cachePolicy,
          // Red
          useProxy:
            typeof options.useProxy === "string" ? options.useProxy : "",
          // Salida
      ...
      async function snapdom(element, userOptions) {
        ...
        console.log(context, userOptions);
      ...
      snapdom.toPng = (el, options) =>
        snapdom(el, { ...options, format: "png" }).then((result) =>
          result.toPng(),
        );
      snapdom.toJpg = (el, options) =>
        snapdom(el, { ...options, format: "jpeg" }).then((result) =>
          result.toJpg(),
        );
      snapdom.toWebp = (el, options) =>
        snapdom(el, { ...options, format: "webp" }).then((result) =>
          result.toWebp(),
        );
      ...
      async function preCache(root = document, options = {}) {
        const { embedFonts = true, cacheOpt = "full", useProxy = "" } = options;
          ...
          if (bg && bg !== "none") {
            ...
            for (const entry of parts) {
              if (entry.startsWith("url(")) {
                const p = Promise.resolve()
                  .then(() =>
                    inlineSingleBackgroundEntry(entry, {
                      ...options,
                      useProxy,
                    }),
                  )
