  ...
  if (node.shadowRoot) {
    ...
    try {
      ...
      for (const s of slots) {
        ...
        try {
          assigned =
            s.assignedNodes?.({ flatten: true }) || s.assignedNodes?.() || [];
        } catch {
  ...
  if (importUrls.length) {
    await Promise.all(
      importUrls.map(
        (u) =>
          new Promise((resolve) => {
            if (document.querySelector(`link[rel="stylesheet"][href="${u}"]`))
              return resolve(null);
...
async function prepareClone(element, options = {}) {
  ...
  if (element === sessionCache.nodeMap.get(clone)) {
    const computed =
      sessionCache.styleCache.get(element) || window.getComputedStyle(element);
...
async function resolveBlobUrlsInTree(root) {
  ...
  for (const node of svgImages) {
    try {
      ...
      const href =
        node.getAttribute("href") || node.getAttributeNS?.(XLINK_NS, "href");
      if (isBlobUrl(href)) {
...
async function inlineBackgroundImages(source, clone, styleCache, options = {}) {
  ...
  while (queue.length) {
    ...
    for (const prop of URL_PROPS) {
      ...
      if (
        inlined.some((p) => p && p !== "none" && !/^url\(undefined/.test(p))
      ) {
...
snapdom.capture = async (el, context, _token) => {
  ...
  const withFormat = (format) => (opts) => {
    ...
    const noBg =
      next.backgroundColor == null || next.backgroundColor === "transparent";
    if (wantsJpeg && noBg) {
...
snapdom.toPng = (el, options) =>
  snapdom(el, { ...options, format: "png" }).then((result) => result.toPng());
snapdom.toJpg = (el, options) =>
  snapdom(el, { ...options, format: "jpeg" }).then((result) => result.toJpg());
snapdom.toWebp = (el, options) =>
  snapdom(el, { ...options, format: "webp" }).then((result) => result.toWebp());
